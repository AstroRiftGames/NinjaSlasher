Shader "UI/Katana Slash Transition Preview"
{
    Properties
    {
        ...
        _CutFeather ("Cut Feather", Float) = 0.0025
        ...
        _HalfSeparation ("Half Separation", Float) = 1.7
        _HalfDrop ("Half Drop", Float) = 0.18
        _HalfDropExponent ("Half Drop Exponent", Float) = 1.45
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _MaxAperture;
            float _HalfSeparation;
            float _HalfDrop;
            float _HalfDropExponent;
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float signedDistance = dot(centeredUv, normal) - _CutPosition;
                float distanceFromCut = abs(signedDistance);

                float softness = max(_EdgeSoftness, 0.0005);
                float cutFeather = max(_CutFeather, 0.00025);
                ...
                float splitProgress = saturate(_OpenProgress);
                float splitEase = splitProgress * splitProgress;
                float splitDistance = _HalfSeparation * splitEase;
                float dropDistance = _HalfDrop * pow(splitProgress, max(_HalfDropExponent, 0.01));
                float2 sharedDrop = float2(0.0, -dropDistance);
                float2 positiveOffset = (normal * splitDistance) + sharedDrop;
                float2 negativeOffset = (-normal * splitDistance) + sharedDrop;
                float2 positiveLocal = centeredUv - positiveOffset;
                float2 negativeLocal = centeredUv - negativeOffset;
                float positiveInside =
                    step(abs(positiveLocal.x), 1.0) *
                    step(abs(positiveLocal.y), 1.0) *
                    smoothstep(-cutFeather, cutFeather, dot(positiveLocal, normal) - _CutPosition);
                float negativeInside =
                    step(abs(negativeLocal.x), 1.0) *
                    step(abs(negativeLocal.y), 1.0) *
                    smoothstep(-cutFeather, cutFeather, _CutPosition - dot(negativeLocal, normal));
                float positiveCoverage = positiveInside;
                float negativeCoverage = negativeInside;
                float blackCoverage = saturate(positiveCoverage + negativeCoverage);
                float blackMask = saturate(_Opacity) * blackCoverage;

                float coreBand = 1.0 - smoothstep(lineThickness, lineThickness + cutFeather, distanceFromCut);
                float glowBand = 1.0 - smoothstep(glowThickness, glowThickness + softness, distanceFromCut);
                ...
                float postTrace = glowBand * _PostTraceIntensity * saturate(_Energy) * slashPresence * (1.0 - (splitProgress * 0.35));
                ...
                float alpha = saturate(blackMask + glowAlpha);
                color.a = alpha;
